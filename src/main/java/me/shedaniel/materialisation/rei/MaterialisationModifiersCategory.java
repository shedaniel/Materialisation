...
public class MaterialisationModifiersCategory implements RecipeCategory<MaterialisationModifiersDisplay> {
    ...
    public List<Widget> setupDisplay(Supplier<MaterialisationModifiersDisplay> recipeDisplaySupplier, Rectangle bounds) {
        ...
        widgets.add(EntryWidget.create(bounds.x + 2, bounds.y + 2).entries(display.getInputEntries().isEmpty() ? Collections.emptyList() : display.getInputEntries().get(0)).markIsInput());
        String title;
        if (level != 1)
        ...
        widgets.add(LabelWidget.create(new Point(bounds.x + 22, bounds.y + 6), title).color(ScreenHelper.isDarkModeEnabled() ? 0xFFBBBBBB : 0xFF404040).leftAligned());
        ...
        for (String descLine : modifier.getModifierDescription(level)) {
            for (String s : textRenderer.wrapStringToWidthAsList(descLine, bounds.width - 4)) {
                widgets.add(LabelWidget.create(new Point(bounds.x + 2, bounds.y + currentY), s).leftAligned().color(ScreenHelper.isDarkModeEnabled() ? 0xFFBBBBBB : 0xFF404040).noShadow());
                currentY += 9;
            }
        }
        return widgets;
