...
public class NetherThingsMaterialsGetter {
    ...
    public static List<MaterialsPack> get() {
        ...
        put(materials, newMat("glowstone").wEnch(12).aIngr(fromItem(tryParse("minecraft:glowstone_dust")), .5f).aIngr(fromItem(tryParse("minecraft:glowstone")), 2).wAtta(1.2f).wFull(120).wSpeed(5f).wDuraMulti(.9f).wSpeedMulti(.9f).setBright(true).wColor(0xfffcc367).setToolDurability(442).setMiningLevel(2));
        put(materials, newMat("vibranium").wEnch(7).aIngr(fromItem(tryParse("netherthings:vibranium")), 2).aIngr(fromItem(tryParse("netherthings:vibranium_block")), 18).wAtta(8).wFull(5463).wSpeed(22).wDuraMulti(1f).wSpeedMulti(1.2f).setBright(false).wColor(0xff8971c4).setToolDurability(21850).setMiningLevel(3));
