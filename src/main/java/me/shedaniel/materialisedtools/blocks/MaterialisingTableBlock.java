package me.shedaniel.materialisedtools.blocks;
...
public class MaterialisingTableBlock extends HorizontalFacingBlock {
    ...
    public boolean activate(BlockState blockState, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hitResult) {
        if (!world.isClient)
            ContainerProviderRegistry.INSTANCE.openContainer(MaterialisedTools.MATERIALISING_TABLE_CONTAINER, player, buf -> buf.writeBlockPos(pos));
        return true;
    }
